...
\begin{alltt}
     (check-equal? (grammar-derive anbncn-csg \elist{})
                   \quot{}(S -> G -> H -> I -> \ep{}))
     (check-equal? (grammar-derive anbncn-csg \quot{}(a a b b c c))
                   \quot{}(S
                     -> ABCS
                     -> ABCABCS
                     -> ABACBCS
...
\scalebox{0.7}{ \begin{tikzpicture}[level distance=1.0cm,level 1/.style={sibling distance=4cm},level 2/.style={sibling distance=4cm},level 3/.style={sibling distance=1.8cm}]
    \node {\texttt{S}}
      ...
      child {node {\texttt{ABCS}}
        child {node {\texttt{ABCG}}
          child {node {\texttt{\textcolor{red}{AB}Gc}}}}
        child {node {\texttt{ABCABCS}}
          child {node {\texttt{ABCABCG}}}
          child {node {\texttt{ABCABCABCS}}}}};
    \end{tikzpicture}}
\end{center}
