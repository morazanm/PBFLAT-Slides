...
\begin{scriptsize}
\begin{itemize}
\item<1-> Alternatively, integrate testing into the constructor

\item<1-> \texttt{\#:accepts} list of words that ought to be accepted

\item<1-> \texttt{\#:rejects} list of words that ought to be rejected

\item<1-> Warning: machine is not built if expected behavior for any word is not met (unlike using check-equal?)
...
(define EVEN-A-ODD-B 
  (make-dfa \quot{}(S M N P)
            ...
            \quot{}((S a P)
              ...
              (P a S)
              (P b M))
            \quot{}no-dead  \textcolor{red}{All tests here, no check-equal? needed}
            #:accepts \quot{}((b) (a a b) (a a a b a b b))
            #:rejects \quot{}(() (a b b a) (b a b b a a)
